...
class Principal extends Person {
    ...
    hireTeacher = function(teacher) {
        this.teachers.push(teacher);
        console.log(this.name + " just hired " + teacher.name);
    }

    recruitStudent = function(student) {
        this.students.push(student);
    }

    expelStudent = function(student) {
        const studentIndex = this.students.findIndex(studentFromArr => studentFromArr.name === student.name && studentFromArr.startYear === student.startYear)
        const studentObject = this.students.splice(studentIndex, 1);
        return studentObject;
    }

    transferStudent = function(student, principle) {
        const studentObject = this.expelStudent(student);
        principle.recruitStudent(studentObject);
    }
